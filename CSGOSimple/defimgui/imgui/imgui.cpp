...
static bool ImGui::BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
	...
	flags |= ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_ChildWindow | ImGuiWindowFlags_NoCollapse;
	...
	if (first_begin_of_the_frame)
	{
		...
		else
		{
			...
			if (!(flags & ImGuiWindowFlags_NoTitleBar))
			{
				...
				window->DrawList->AddRectFilled(title_bar_rect.Min + ImVec2(0, 20), title_bar_rect.Max, GetColorU32(ImGuiCol_SliderGrabActive));
			}
			...
			if (flags & ImGuiWindowFlags_MenuBar)
			{
				...
				if (style.FrameBorderSize > 0.0f && menu_bar_rect.Max.y < window->Pos.y + window->Size.y)
					window->DrawList->AddLine(menu_bar_rect.GetBL(), menu_bar_rect.GetBR(), GetColorU32(ImGuiCol_SliderGrabActive), style.FrameBorderSize);
			}
		...
		if (!(flags & ImGuiWindowFlags_NoTitleBar))
		{
			...
			RenderTextClipped(text_r.Min + ImVec2(1, 1), text_r.Max + ImVec2(1, 1), name, NULL, &text_size, style.WindowTitleAlign, &clip_rect);
			PopStyleColor();
