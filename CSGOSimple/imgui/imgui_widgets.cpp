...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	if (hovered)
		window->DrawList->AddRect(check_bb.Min, check_bb.Max, ImColor(79, 79, 90, 255));
	else
		window->DrawList->AddRect(check_bb.Min, check_bb.Max, ImColor(57, 57, 68, 255));

	if (*v)
	{
		const float pad = ImMax(1.0f, (float)(int)(square_sz / 5.0f));
		window->DrawList->AddRectFilled(check_bb.Min + ImVec2(pad, pad), check_bb.Max - ImVec2(pad, pad), ImGui::GetColorU32(ImGui::GetStyleColorVec4(ImGuiCol_SliderGrabActive)));
	}
	...
	if (label_size.x > 0.0f) {
		...
		RenderText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x + 1, check_bb.Min.y + style.FramePadding.y + 1), label);
		PopStyleColor();
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
	...
	if (hovered)
		window->DrawList->AddRect(frame_bb.Min + ImVec2(0, 5), frame_bb.Max, ImColor(79, 79, 90, 255));
	else
		window->DrawList->AddRect(frame_bb.Min + ImVec2(0, 5), frame_bb.Max, ImColor(57, 57, 68, 255));

	if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
		RenderTextClipped(frame_bb.Min + style.FramePadding + ImVec2(0, 3), ImVec2(value_x2, frame_bb.Max.y), preview_value, NULL, NULL, ImVec2(0.0f, 0.0f));
	if (label_size.x > 0) {
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power)
{
	...
	{
		const auto col = ImGui::GetColorU32(ImGui::GetStyleColorVec4(ImGuiCol_SliderGrabActive));

		// slider grab
		window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(2, 7), grab_bb.Max, col, style.FrameRounding);
	}
	// Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
	char value_buf[64];
	const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, v, format);
	PushStyleColor(ImGuiCol_Text, ImVec4(0.f, 0.f, 0.f, 1.f));
	RenderText(ImVec2(frame_bb.Max.x - CalcTextSize(value_buf).x - 1.0f + 1, total_bb.Min.y + style.FramePadding.y + 1), value_buf, value_buf_end);
	PopStyleColor();
	PushStyleColor(ImGuiCol_Text, ImVec4(151 / 255.f, 151 / 255.f, 151 / 255.f, 1.f));
	...
	if (label_size.x > 0.0f) {
		PushStyleColor(ImGuiCol_Text, ImVec4(0.f, 0.f, 0.f, 1.f));
		RenderText(ImVec2(frame_bb.Min.x + 1, total_bb.Min.y + style.FramePadding.y + 1), label);
		PopStyleColor();
		RenderText(ImVec2(frame_bb.Min.x, total_bb.Min.y + style.FramePadding.y), label);
	}
	return value_changed;
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
	...
	if (selected)
		PushStyleColor(ImGuiCol_Text, g.Style.Colors[ImGuiCol_SliderGrabActive]);
	else if (hovered && !selected)
		PushStyleColor(ImGuiCol_Text, ImVec4(79, 79, 90, 255));
	else
		PushStyleColor(ImGuiCol_Text, ImVec4(57, 57, 68, 255));
